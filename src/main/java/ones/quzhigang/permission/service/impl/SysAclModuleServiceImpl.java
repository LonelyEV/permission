...
public class SysAclModuleServiceImpl implements SysAclModuleService {
    ...
    public void delById(long id) {
        ...
        Preconditions.checkNotNull(sysAclModuleModel, "待删除的权限模块不存在，无法删除");

        if(sysAclModuleMapper.countByParentId(sysAclModuleModel.getId()) > 0){
            throw new PermissionException("当前权限模块下存在子模块，无法删除");
        }
        if(sysAclMapper.countByAclModuleId(Integer.valueOf(String.valueOf(sysAclModuleModel.getId()))) > 0){
            throw new PermissionException("当前权限模块下已存在用户，无法删除");
        }
